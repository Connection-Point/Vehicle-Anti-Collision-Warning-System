...
def signalDetection(indexesLights , boxesLights , image_np , signalCounter , flagSignal):
  ...
  # RETR = cv2.findContours(maskRed.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
  # print (RETR)
  ...
  if sum(flagSignal) > 5:
    cv2.putText(image_np, "Red Light Detected, Stay Put", (340,160), font, 1.2, (0,0,255), 2,cv2.LINE_AA)
    signalCounter = 1
  else:
    signalCounter = signalCounter - 1
  if -16 < signalCounter <= 0:
    cv2.putText(image_np, "You're Good to Go", (400,160), font, 1.2, (0,255,255), 2,cv2.LINE_AA)
