...
def estimate_stepping(indexesPersons , boxesPersons , image_np , flagPerson , areaPerson , areaDetails):
	...
	if flagPerson > 0:
		...
		sound_played = False
		if areaPerson > 15000:
			cv2.putText(image_np,"Caution, Collision Might Imminent " ,(270,120), font, 1.2,(0,0,255),2,cv2.LINE_AA)
			if not sound_played:
			...
			cv2.putText(image_np,"People Detected Nearby, Stay Slow "    ,(290,120), font, 1.2,(0,255,255),2,cv2.LINE_AA)
			sound_played = False
			if not sound_played:
				# play the sound in a separate thread
