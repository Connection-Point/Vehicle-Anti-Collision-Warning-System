...
model_name = 'models/ssd_mobilenet_v1_fpn_shared_box_predictor_640x640_coco14_sync_2018_07_03'
model_dir =  "models/ssd_mobilenet_v1_fpn_shared_box_predictor_640x640_coco14_sync_2018_07_03/saved_model"
...
def selectRegions(image  , text , flag):
    ...
    while True:
        ...
        if flag==1:
            ...
            cv2.putText(image, "Press 'r' to Reset Your Selection of ROI.",  (290,70), font , 1.2, [0,255,255], 2,cv2.LINE_AA)
            cv2.putText(image, "Press 'd' When the Selection of ROI is Done.",  (180,110), font , 1.2, [0,255,255], 2,cv2.LINE_AA)
        else:
            cv2.putText(image, text ,  (240,30), font , 1.2, [0,255,0], 2,cv2.LINE_AA)
            cv2.putText(image, "Press 'r' to Reset Your Selection of ROI.",  (290,70), font , 1.2, [0,255,0], 2,cv2.LINE_AA)
            cv2.putText(image, "Press 'd' When the Selection of ROI is Done.",  (180,110), font , 1.2, [0,255,0], 2,cv2.LINE_AA)
...
def night():
    ...
    Quit = selectRegions(image  , "Please Click Point to Select Your Dashcam Postision in Vehicle." , 1)
    dashPointer = refPt
...
def day():
    ...
    Quit = selectRegions(copy.deepcopy(image)  , "Click Point to Select Your Vehicle Dashcam" , 1)
    dashPointer = refPt
    if len(dashPointer) <= 2:
        dashPointer = [[0,0], [0,0], [0,0]]
    refPt = []
    ...
    Quit = selectRegions(copy.deepcopy(image)  , "Click Point to Select a Bird's Eye View (Road)" , 2)
    lanePointer = refPt
...
cap=cv2.VideoCapture('videos/test.mp4')  # Video Path
start_frame = 0*24
